...
def test_401_get_response_with_body(client):
    ...
    assert "Authentication failed for 'GET https://fake.example.com/example'" in str(exc_info.value)
...
def test_401_post_response_with_body(client):
    ...
    # Check that the exception contains the response body
    assert "Authentication failed for 'POST https://fake.example.com/example'" in str(exc_info.value)
    assert error_body in str(exc_info.value)
    assert exc_info.value.response is not None
    assert exc_info.value.response.status_code == 401

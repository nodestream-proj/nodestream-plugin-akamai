...
class AkamaiEdgeWorkersClient(AkamaiApiClient):
    ...
    def get_production_version(self, edgeworker_id):
        get_production_version_path = (
            f"/edgeworkers/v1/ids/{edgeworker_id}/activations?network=PRODUCTION&activeOnNetwork=true"
        )
        activations = self._get_api_from_relative_path(get_production_version_path)["activations"]
